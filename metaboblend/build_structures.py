...
def annotate_msn(msn_data: Dict[str, Dict[str, Union[int, list]]],
                 ...
                 heavy_atoms: Union[Sequence[int], None] = None, max_atoms_available: int = 2, max_degree: int = 6,
                 ...
                 ncpus: Union[int, None] = None, write_fragment_smis: bool = False,
    ...
    :param path_smi_out: The directory to which unique smile strings should be written representing the final
        ...
        and the number of `prescribed_masses` by which each structure was generated. If None, no files are written.
        See `write_fragment_smis` should you wish to inspect the SMILEs generated by individual peaks.
    ...
    :param write_fragment_smis: Whether to write smiles to a file for each fragment. Requires `path_smi_out` to be
        specified. If `True`, a new directory is generated for each input spectral tree and the results for each
        fragment are stored as a text file that contains unique smiles strings.
    ...
    for i, ms_id in enumerate(msn_data.keys()):
        if path_smi_out is not None and write_fragment_smis:
            path_smi_out_subdir = os.path.join(path_smi_out, str(i+1) + "_" + ms_id)
            os.mkdir(path_smi_out_subdir)
        else:
            path_smi_out_subdir = None
        ...
        for prescribed_mass in msn_data[ms_id]["fragment_masses"]:
            if path_smi_out_subdir is not None and write_fragment_smis:
                id_path_smi_out = os.path.join(path_smi_out_subdir, str(round(prescribed_mass, 4)) + ".smi")
                open(id_path_smi_out, "w").close()
            else:
        ...
        if path_smi_out is not None:
            with open(os.path.join(path_smi_out, ms_id + "_frequency.csv"), "w") as freq_out:
                freq_out.writelines(
...
def generate_structures(ms_data: Dict[str, Dict[str, Union[int, None]]],
                        ...
                        max_degree: int = 6, max_atoms_available: int = 2, max_n_substructures: int = 3,
                        path_smi_out: Union[str, bytes, os.PathLike, None] = None, ncpus: Union[int, None] = None,
...
def gen_subs_table(db, heavy_atoms, max_degree, max_atoms_available, max_mass, table_name="subset_substructures",
