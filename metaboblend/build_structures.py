...
def combine_ecs(precise_mass_grp, db, table_name, accuracy, ppm=None):
    """
    ...
    for i in range(len(precise_mass_grp)):
        atoms = db.select_ecs(precise_mass_grp[i], table_name, accuracy, ppm=ppm)
...
def build(mc, exact_mass, heavy_atoms, max_valence, max_atoms_available, max_n_substructures, smi_out,
    ...
    for ss_grp in subsets:
        ...
        for ss_grp2 in subsets_r2:  # refines groups based on ecs and gets substructures from db (appends to lls)
            lls += build_from_subsets(ss_grp2, mc=mc, table_name=table_name, ppm=ppm, db=db)
...
def build_from_subsets(ss2_grp, mc, table_name, db, ppm=None):
    """
