...
class ResultsDb:
    ...
    def add_results(self, ms_id, smi_dict, fragment_mass=None, fragment_id=None):
        """
    ...
    for i, ms_id in enumerate(msn_data.keys()):
        ...
        for j, fragment_mass in enumerate(msn_data[ms_id]["fragment_masses"]):
            ...
            for k in range(0 - hydrogenation_allowance, hydrogenation_allowance + 1):
                ...
                results_db.add_results(ms_id, smi_dict, fragment_mass, j)
                fragment_smis = None

        if yield_smi_dict:
            yield {ms_id: results_db.get_structures(ms_id)}
...
def generate_structures(ms_data: Dict[str, Dict[str, Union[int, None]]],
    ...
    :param path_sql_out: The path to the SQLite 3 substructure database, as generated by
        :py:meth:`metaboblend.databases.SubstructureDb`.
    ...
    db = SubstructureDb(path_substructure_db, path_connectivity_db)
    results_db = ResultsDb(path_sql_out, False)
    results_db.create_results_db()
    ...
    results_db.add_ms(ms_data, [None, ha_min, ha_max, max_atoms_available, max_degree, max_n_substructures,
                                None, isomeric_smiles])
    for ms_id in ms_data.keys():
        ...
        results_db.add_results(ms_id, smi_dict, ms_data[ms_id]["prescribed_masses"])
