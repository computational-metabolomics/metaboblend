...
def annotate_msn(msn_data: Dict[str, Dict[str, Union[int, list]]],
                 path_substructure_db: Union[str, bytes, os.PathLike], ppm: int = 5,
                 ...
                 path_connectivity_db: Union[str, bytes, os.PathLike, None] = os.path.join(
                     os.path.realpath(os.path.dirname(__file__)), "data", "connectivity.sqlite"),
    ...
    """
    :param msn_data: Dictionary in the form msn_data[id] =
        {mf: [C, H, N, O, P, S], exact_mass: float, fragment_masses=[]}. id represents a unique identifier for
        a given  spectral tree or fragmentation spectrum, mf is a list of integers referring to the molecular formula
        of the structure of interest, exact_mass is the mass of this molecular formula to >=4d.p. and prescribed_masses
        are neutral fragment masses generated by this structure used to inform candidate scoring.
    ...
    :param heavy_atoms: Restricts the size (number of heavy atoms) of substructures to be used
        to build final structures. If None, no limit is applied.

    :param max_valence: The maximal total bond orders of substructures to be considered to build final structures
        (ie, the product of `atoms_available` and the degree of their bonds). Maximum valence is also limited by the
        extensivity of the supplied connectivity database.

    :param max_atoms_available: The maximal atoms available of substructures to be considered for building molecules.
        atoms_available refers to the number of atoms on a substructre involved in bonding. Atoms available are also
        limited by the extensivity of the supplied connectivity database.
    ...
    :param add_small_substructures: Whether to add a curated list of small substructures (single atom) to the filtered
        database prior to annotation. The `heavy_atoms` parameter must include 1 in order to make use of these
        additional substructures. It is recommended that these substructures are added because i) the substructure
        generation methods of MetaboBlend do not allow for the generation of substructures with less than 2 heavy atoms
        and ii) these substructures are commonly required for building the correct structure from fragment masses.

    :param smi_out_dir: The path of the file to which unique smile strings should be written representing the final
        structures generated. A new directory is generated for each input molecule and the results are stored within
        this directory; a csv file will be generated with the unique SMILEs strings of structures built and
        the number of `prescribed_masses` by which each structure was generated. If None, no files are written. See
        `write_fragment_smis` should you wish to inspect the SMILEs generated by individual peaks.
    ...
    :param path_connectivity_db: The path to the SQLite 3 connectivity database, as generated by
        ...
        databased bundled with MetaboBlend will be used.
    ...
    :param write_fragment_smis: Whether to write smiles to a file for each fragment. Requires `smi_out_dir` to be
        specified. Not recommended unless inspection of the results from individual peaks is necessary.
    ...
    :param yield_smi_dict: If True, for each input molecule the function yields a dictionary whose keys are SMILEs
        strings and values are the number of `prescribed_masses` by which the structure was generated.

    :return: At default, for each input molecule yields a dictionary whose keys are SMILEs strings for the generated
        structures and values are the number of `prescribed_masses` by which the structure was built.
    """
    ...
    for i, ms_id in enumerate(msn_data.keys()):
        if smi_out_dir is not None and write_fragment_smis:
            smi_out_path_subdir = os.path.join(smi_out_dir, str(i+1) + "_" + ms_id)
        ...
        msn_data[ms_id]["fragment_masses"].sort(reverse=True)

        for prescribed_mass in msn_data[ms_id]["fragment_masses"]:
            if smi_out_path_subdir is not None and write_fragment_smis:
                smi_out_path = os.path.join(smi_out_path_subdir, str(round(prescribed_mass, 4)) + ".smi")
        ...
        if smi_out_dir is not None:
            with open(os.path.join(smi_out_dir, ms_id + "_frequency.csv"), "w") as freq_out:
                freq_out.writelines(
...
def generate_structures(msn_data: Dict[str, Dict[str, Union[int, list, None]]],
                        path_substructure_db: Union[str, bytes, os.PathLike],
                        heavy_atoms: Union[Sequence[int], None] = range(2, 9),
                        ...
                        path_connectivity_db: Union[str, bytes, os.PathLike, None] = os.path.join(
                            os.path.realpath(os.path.dirname(__file__)), "data", "connectivity.sqlite"),
                        minimum_frequency: Union[int, None] = None, yield_smi_set: bool = True
    ...
    written in text format. For the generation of structures from MSn data, see
    :py:meth:`metaboblend.build_structures.annotate_msn`.

    :param msn_data: Dictionary in the form ms_data[id] =
        {mc: [C, H, N, O, P, S], exact_mass: float, prescribed_masses=[]}. id represents a unique identifier for
    ...
    :return: For each input molecule, yields a set of unique SMILEs strings generated by the build method (unless
        `yield_smi_set = False`)
    """
    ...
    for ms_id in msn_data.keys():
        ...
        ppm = None
        try:
            if msn_data[ms_id]["prescribed_masses"] is not None:
                ppm = 0
        except KeyError:
            msn_data[ms_id]["prescribed_masses"] = None
        ...
        smi_list = build(
            ...
            path_connectivity_db=path_connectivity_db,
            path_substructure_db=path_substructure_db,
            prescribed_mass=msn_data[ms_id]["prescribed_masses"],
            ppm=ppm,
            out_mode="w",
            table_name=table_name,
            processes=processes,
...
def build(mf, exact_mass, max_n_substructures, smi_out_path, path_connectivity_db, path_substructure_db,
    ...
    molecules are written to the specified file or returned in smiles format. This function represents the central
    building method used by MetaboBlend.
