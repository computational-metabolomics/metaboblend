...
def calculate_complete_multipartite_graphs(max_atoms_available: int, max_n_substructures: int):
    """
...
def get_degrees(max_atoms_available, ug):
    ...
    :param max_atoms_available: The maximal number of atoms available in each substructure for bonding. At least one
        atom must be available for bonding for a graph to be created.

    :param ug: :py:meth:`networkx.Graph` graph generated by geng and labelled using the mappings calculated by RI.

    :return: The degree available for each substructure in the graph.
    """
...
def draw_subgraph(edges, vn):
    """
    Generates a visual representation of a connectivity subgraph; this graph is generated by
    geng and labelled using the mappings provided by RI.

    :param edges: String containing the edges of the connectivity subgraph.

    :param vn: String containing the degrees (number of atoms available for bonding) of the subgraph's substructures
        (nodes), as generated by :py:meth:`metaboverse.auxiliary.get_degrees`.

    :return: A :py:meth:`pylab.plt` visualisation and `networkx.Graph' object of the connectivity subgraph.
    """
...
def graph_to_ri(graph, name):
    """
    Converts a :py:meth:`networkx.Graph' to gfu format to be read by RI. Allows for calculation of mappings by RI which relate
    the output of geng to the complete multipartite graphs generated by
    :py:meth:`metaboverse.auxiliary.calculate_complete_multipartite_graphs`.

    :param graph: Graph generated by networkx (e.g. one generated by
        :py:meth:`metaboverse.auxiliary.calculate_complete_multipartite_graphs` or geng).
...
def graph_info(p, sG, mappings):
    ...
    :return: A dictionary whose keys contain substructure valences and objects is a list of lists containing the
...
def sort_subgraphs(subgraphs):
    """
    Sorts a lists of edges for a given connectivity subgraph, provided by :py:meth:`metaboverse.auxiliary.graph_info`,
    ...
    :param subgraphs: A list of lists containing a tuple of the possible sets of edges between substructures in a
        connectivity graph, created by :py:meth:`metaboverse.auxiliary.graph_info`.
