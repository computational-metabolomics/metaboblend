...
def test_build(out_dir, mc, exact_mass, mol, hmdb_id, path_subs, path_k_graphs, path_pkls, heavy_atoms, max_valence,
    ...
    num_generated_peaks = 0
    initial_stage = True
    possible_structures = None
...
=======
                  table_name="msn_subset", possible_structures=possible_structures)

        substructures_were_generated = False
>>>>>>> 73498ecf6599bd68aa1a4ae75c655418dcff9010
        get_uniq_subs(smi_out, ignore_substructures=True)
        with open(smi_out, mode="r") as smis:
            for line in smis:
                if len(line) > 0:
                    substructures_were_generated = True
                    try:
                        pre_reccurence[line.strip()] += 1
                    except KeyError:
                        pre_reccurence[line.strip()] = 1

        if substructures_were_generated:
            num_generated_peaks += 1  # if structures were generated this round, increase peak counter

        # if we're still in stage 1 and we hit the number of peaks required to subset future structures
        if initial_stage & (num_generated_peaks == peaks_to_select_structures):
            initial_stage = False
            possible_structures = set(pre_reccurence.keys())  # set the possible structures
...
def run_test(out_dir, ms_data, test_name, heavy_atoms, max_valence, accuracy, ppm, db_path, test_type="ind_exp",
             subset=True, max_atoms_available=2, minimum_frequency=None, peaks_to_select_structures=20):
    """
    ...
    if subset:
        ...
        if test_type == "ind_exp":
            C(out_dir, ms_data, heavy_atoms, max_valence, accuracy, ppm, db_path,
              ...
              peaks_to_select_structures=peaks_to_select_structures)
...
def C(out_dir, ms_data, heavy_atoms, max_valence, accuracy, ppm, db_path, max_atoms_available=2,
      minimum_frequency=None, peaks_to_select_structures=20):
